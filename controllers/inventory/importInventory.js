...
const importInventory = async (req, res) => {
  try {
    ...
    try {
      ...
      // Handle ObjectId mapping for returned inventory (if MongoDB_ObjectId exists in Excel)
      let updatedReturnedInventoryEntries = 0;
      try {
        // Create mapping between old and new ObjectIds
        const idMappings = [];
        for (const newItem of importResult) {
          const excelRow = jsonData.find(row => row.ID === newItem.id);
          if (excelRow?.MongoDB_ObjectId) {
            idMappings.push({
              oldObjectId: excelRow.MongoDB_ObjectId,
              newObjectId: newItem._id.toString()
            });
          }
        }

        // Update returned inventory references if mappings exist
        if (idMappings.length > 0) {
          console.log(`Processing ${idMappings.length} ObjectId mappings for returned inventory...`);

          for (const mapping of idMappings) {
            const updateResult = await ReturnedInventory.updateMany(
              {
                'items.item': mapping.oldObjectId,
                status: 'pending'
              },
              {
                $set: { 'items.$.item': mapping.newObjectId }
              }
            );
            updatedReturnedInventoryEntries += updateResult.modifiedCount;
          }

          console.log(`Updated ${updatedReturnedInventoryEntries} returned inventory entries with new ObjectIds`);
        }
      } catch (mappingError) {
        console.log('ObjectId mapping not performed (Excel may not contain MongoDB_ObjectId column):', mappingError.message);
      }
      ...
      res.json({
        ...
        updatedReturnedInventoryEntries: updatedReturnedInventoryEntries,
